...
**Размеры шрифтов**:
- Заголовок на обложке: 26pt (Fairy/DejaVu Bold)
- Заголовок главы: 26pt Fairy или 23pt DejaVu Bold
- Тело текста: 15pt Fairy или DejaVu Regular
...
**Символы**: `✦` (U+2726) заменяется на `◆` (U+25C6) в `_fix_common_grammar()` — DejaVu и Neucha не содержат U+2726.
...
**Архетипы сюжетов** (`_PLOT_ARCHETYPES`): 15 уникальных архетипов, случайно выбирается один на каждую генерацию. Каждый содержит завязку, обязательный поворот и список запрещённых клише. Это обеспечивает разнообразие сюжетов для разных детей.

**Пост-обработка** (`_fix_common_grammar`):
- `остановилася` → `остановилась`, `вернулося` → `вернулось` (рефлексивные глаголы)
- `Глава первый/второй/...` → `Глава первая/вторая/...` (глава — женского рода)
- `✦` → `◆`

**Падежи имён** (`_name_gen`, `_to_genitive`):
- После мягких/шипящих/г,к,х,ц: "а" → "и" (Маша→Маши, Наташа→Наташи)
- После остальных согласных: "а" → "ы" (Лера→Леры, Нина→Нины, Алина→Алины)

**Консистентность вторичных персонажей**: промпт требует фиксированного текстового описания внешности для каждого именованного вторичного персонажа, который появляется в нескольких image prompts. Это дополнение к pixel-reference механизму Together AI (который работает только для главного героя).
...
⚠️ **Внимание при `git pull`**: если на сервере есть незакоммиченные изменения в tracked-файлах — делать `git stash` перед pull, потом `git stash pop`. Конфликты в story_service.py решать через `git checkout --theirs`.
...
- [ ] `cost_guard.py` — allowlist моделей проверяется при старте backend; если в .env незнакомая модель — сервер не запустится (нужно добавить модель в allowlist)
- [ ] Некоторые грамматические ошибки LLM (неестественные фразы, падежи у нарицательных существительных) не поддаются автоматическому исправлению — это качество модели
