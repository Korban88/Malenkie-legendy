...
`story_service.py` импортирует из стандартных файлов (image_service, pdf_service, text_service) — это правильно.
...
**Главная идея**: обложка генерируется первой, затем все следующие иллюстрации получают URL обложки как `image_urls` — FLUX.2-pro буквально смотрит на обложку при генерации следующих сцен.

```
i=0 (обложка):    FLUX.1.1-pro, без reference → сохраняется как cover_public_url
i=1,2,3,4:        FLUX.2-pro + image_urls=[cover_public_url] → строгая консистентность
```

**Итого**: 5 изображений — 1 обложка + 4 иллюстрации (по одной на каждую главу).
...
### Стили изображений (`_IMG_STYLE_SUFFIX`)

Каждый стиль содержит точный визуальный референс:

| Стиль | Референс |
|---|---|
| `watercolor` | Реальная акварель на бумаге — зернистость, wet-on-wet, прозрачные слои, Beatrix Potter |
| `soviet` | Чебурашка (1966), Ну Погоди (1969), Кот Леопольд — контур, плоская заливка, охра-терракота |
| `ghibli` | Мой сосед Тоторо, Унесённые призраками — пышные фоны, мягкие персонажи, золотой свет |
| `disney` | Красавица и Чудовище (1991), Русалочка — элегантные контуры, jewel-tone палитра |
| `pixar` | Вверх, Тайна Коко — 3D CGI, subsurface rendering, кинематографичный свет |
| `cartoon` | Классический американский мультфильм — чистые линии, плоская заливка |
| `storybook` | Eric Carle / Maurice Sendak — тушь + акварель, тёплая палитра |

**Строгое правило**: в каждом промпте — ТОЛЬКО персонажи из текста данной главы. Никаких случайных стариков, гномов, животных, которых нет в тексте.
...
**Размеры шрифтов**:
- Заголовок на обложке: 26pt
- Заголовок главы: 26pt Fairy или 23pt DejaVu Bold
- Тело текста: 15pt
- После рендера заголовка главы явно вызывается `_body_font()` чтобы шрифт не наследовался

**Парсинг глав**: `para.startswith('Глава')` — если LLM поставил `\n` (не `\n\n`) между названием и первым абзацем, берётся только первая строка как заголовок, остаток идёт в body.

**Карта иллюстраций** (`_CHAPTER_IMG_MAP`): 4 главы, каждая получает свою картинку:
```python
_CHAPTER_IMG_MAP = {0: 1, 1: 2, 2: 3, 3: 4}  # chapter_index → image_index
# image[0] = обложка, image[1..4] = иллюстрации глав 1..4
```

**Символы**: `✦` (U+2726) заменяется на `◆` (U+25C6) — DejaVu и Neucha не содержат U+2726.

---

## 8. Генерация текста — text_service.py

**Провайдер**: OpenRouter (gpt-4o-mini), fallback → OpenAI direct, fallback → template.

**Структура сказки**: **4 главы** (8000–9500 символов). LLM генерирует 5 image prompts: обложка + по одному на каждую из 4 глав. `story_service.py` вызывает `generate_images(count=5)`.

**Архетипы сюжетов** (`_PLOT_ARCHETYPES`): 15 уникальных архетипов, случайно выбирается один на каждую генерацию. Каждый содержит завязку, обязательный поворот и список запрещённых клише (чтобы не повторялись «мост испытаний», «мудрый старик», «Страж» и т.д.).

**Пост-обработка** (`_fix_common_grammar`):
- `остановилася` → `остановилась`, `вернулося` → `вернулось`
- `Глава первый/второй/...` → `Глава первая/вторая/...`
- `✦` → `◆`

**Падежи имён** (`_name_gen`, `_to_genitive` в pdf_service):
- После шипящих/г,к,х,ц + «а»: → «и» (Маша→Маши, Наташа→Наташи)
- После остальных согласных + «а»: → «ы» (Лера→Леры, Нина→Нины)

**Консистентность вторичных персонажей**: промпт требует фиксированного описания внешности для каждого именованного вторичного персонажа (одинаковые слова в каждом промпте где он появляется).

**Запрет лишних персонажей**: каждый image prompt содержит явный запрет добавлять персонажей, которых нет в тексте данной главы.

---

## 9. Telegram Stars платёж
...
⚠️ **Проблема при `git pull`**: сервер часто имеет staged/unstaged изменения в `story_service.py`. Решение:
```bash
git restore --staged backend/app/services/story_service.py
git checkout -- backend/app/services/story_service.py
git pull origin main
```

---

## 11. Архитектура запроса
...
Telegram → bot.py → POST /stories/generate (FastAPI)
              ...
              │    image_prompts[5], title    │
              │                               │
              │ 2. image_service: Together AI │
              │    5 PNG → /storage/images/   │
              │    (обложка + 4 главы)        │
              │                               │
              │ 3. pdf_service: fpdf2         │
              │    PDF → /storage/stories/    │
...
- [ ] `force_episode_one=True` — всегда генерирует эпизод 1. Для продакшена переключить на `False`
- [ ] Stability AI ключ без кредитов — убрать из .env или пополнить
- [ ] Статичные UI-картинки (`/opt/malenkie-legendy/static/ui/*.png`) — не в репозитории
- [ ] `cost_guard.py` — allowlist моделей проверяется при старте; если в .env незнакомая модель — сервер не запустится
- [ ] Некоторые грамматические ошибки LLM (неестественные фразы, сложные падежи) не поддаются авто-исправлению

---

## 13. Частые команды для отладки
