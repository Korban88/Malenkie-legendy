...
_IMG_STYLE_FOR_PROMPT = {
    'watercolor': 'traditional watercolor on cold-press paper — visible paper grain, transparent pigment washes, wet-on-wet bleeds, white paper showing through highlights, Beatrix Potter / Quentin Blake illustration quality',
    'ghibli':     'Studio Ghibli hand-drawn 2D anime — exact style of My Neighbor Totoro and Spirited Away: lush painterly backgrounds, soft rounded characters with large gentle eyes, warm earthy palette, golden dappled light',
    'soviet':     'Soviet Soyuzmultfilm 2D cel animation — exact style of Cheburashka (1966) and Nu Pogodi (1969): bold black outlines, flat color fills, warm muted ochre-terracotta-sage palette, simple rounded shapes, retro USSR aesthetic',
    'disney':     'classic Disney 2D animation — exact style of Beauty and the Beast (1991) and The Little Mermaid (1989): elegant fluid outlines, vibrant jewel-tone colors, sparkling magical light, graceful expressive characters',
    'pixar':      'Pixar 3D CGI animation — exact quality of Up and Coco: polished subsurface rendering, soft volumetric light, expressive large eyes, richly textured environments, warm cinematic color grading',
    'cartoon':    'classic flat cartoon — bold uniform black outlines, solid color fills, minimal shading, high-contrast saturated palette, clean vector-like clarity',
    'storybook':  "classic published picture book illustration — controlled ink linework with watercolor fill, warm earthy palette, rich storytelling composition, Eric Carle / Maurice Sendak quality",
}
...
def _prompt(payload: dict, archetype: dict | None = None) -> str:
    ...
    return (
        ...
        'ТРЕБОВАНИЯ К ТЕКСТУ:\n'
        '1) story_text — СТРОГО 8000–9500 символов. Минимум 17 минут чтения вслух.\n'
        '2) Структура: РОВНО 4 главы. Каждая — минимум 5–7 абзацев по 4–5 предложений. НЕ СОКРАЩАЙ.\n'
        '3) Глава начинается: "Глава [номер словами]. [Название]" — ЖЕНСКИЙ РОД: «первая», «вторая», «третья», «четвёртая».\n'
        ...
        'ТРЕБОВАНИЯ К IMAGE PROMPTS (РОВНО 5 штук: обложка + по одной на каждую из 4 глав):\n'
        'Язык промптов — английский. Длина каждого — 45–65 слов.\n'
        f'КАЖДЫЙ промпт — КОНКРЕТНАЯ ЖИВАЯ СЦЕНА из текста данной главы: кто что делает, где, '
        f'{animal} активно участвует в каждом кадре. '
        'НЕЛЬЗЯ описывать внешность главного героя — она добавится автоматически.\n\n'
        'СТРОГОЕ ПРАВИЛО — ТОЛЬКО ПЕРСОНАЖИ ИЗ ТЕКСТА ЭТОЙ ГЛАВЫ:\n'
        'В каждом промпте изображай ТОЛЬКО тех персонажей, которые явно присутствуют в данной главе. '
        'КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО добавлять: случайных стариков, старушек, гномов, эльфов, '
        'волшебников, животных, существ — если они НЕ упомянуты в тексте этой главы. '
        f'Если в главе есть только {name} и {animal} — на картинке ТОЛЬКО они двое. '
        'Ни одного лишнего персонажа.\n\n'
        'ВТОРИЧНЫЕ ПЕРСОНАЖИ — СТРОГАЯ КОНСИСТЕНТНОСТЬ МЕЖДУ ПРОМПТАМИ:\n'
        'Если именованный вторичный персонаж появляется в нескольких главах — '
        'ПРИДУМАЙ ему фиксированное описание (пол, возраст, цвет волос, одежда) '
        'и вставляй ДОСЛОВНО одни и те же слова в КАЖДЫЙ промпт с этим персонажем. '
        'Пример: "a boy age 8 with short dark curly hair wearing a green jacket" — '
        'идентичная фраза в каждом промпте.\n\n'
        ...
        f'[0] ОБЛОЖКА — epic wide establishing shot: {name} в кульминационный момент всей сказки, '
        f'{animal} рядом активно участвует, окружение "{place}" богато и атмосферно. '
        f'Динамичная диагональная композиция, магическое освещение.\n\n'
        f'[1] ГЛАВА 1 — wide establishing shot: точная сцена из первой главы, '
        f'первое волшебное событие, {name} и {animal} вместе открывают что-то удивительное. '
        f'Только персонажи главы 1. Детальное окружение "{place}", эмоция удивления.\n\n'
        f'[2] ГЛАВА 2 — medium wide shot: конкретная сцена из второй главы. '
        f'Только персонажи главы 2 ({animal} участвует). '
        f'Атмосфера нарастающего интереса или первое препятствие.\n\n'
        f'[3] ГЛАВА 3 — wide shot: ключевая сцена третьей главы — момент с другими персонажами '
        f'ИЗ ТЕКСТА ГЛАВЫ 3. Только те, кто есть в этой главе. '
        f'{name} и {animal} в центре, напряжение или важное взаимодействие.\n\n'
        f'[4] ГЛАВА 4 / ФИНАЛ — dynamic wide-angle shot: кульминация и победа из четвёртой главы. '
        f'Только персонажи финальной главы. Золотой или магический свет, '
        f'"{place}" на пике красоты, ощущение завершения.\n\n'
        f'КРИТИЧНО: 5 промптов = 5 РАЗНЫХ ОСВЕЩЕНИЙ + 5 РАЗНЫХ РАКУРСОВ + '
        f'{animal} действует в каждом + "{place}" чётко виден в каждом кадре. '
        'Никаких описаний внешности главного героя — только сцена, мир, действие!\n\n'
        ...
        'ГРАММАТИКА — АБСОЛЮТНЫЙ ПРИОРИТЕТ. КАЖДОЕ ПРЕДЛОЖЕНИЕ ОБЯЗАТЕЛЬНО ПРОВЕРЯЕТСЯ.\n\n'
        ...
        f'• ЖИВОТНОЕ-СПУТНИК «{animal}»: пол — ГРАММАТИЧЕСКИЙ РОД СЛОВА, а НЕ род героя. '
        f'«волк» — мужской → "сказал волк"; «лиса» — женский → "сказала лиса". '
        f'НИКОГДА не путай род героя с родом животного.\n\n'
        'СТРОГО ЗАПРЕЩЁННЫЕ КОНСТРУКЦИИ — ПЕРЕПИСЫВАЙ НЕМЕДЛЕННО:\n'
        '• «-лася»: «остановилася», «улыбнулася», «вернулася» — НЕТ ТАКИХ СЛОВ. '
        'Только «остановилась», «улыбнулась», «вернулась».\n'
        '• «-лося»: «вернулося» — НЕТ. Только «вернулось».\n'
        '• «она говорил», «он сказала» — глагол ОБЯЗАН согласовываться с подлежащим.\n'
        '• «Наконец-то, — сказала волк» — «волк» м.р. → «сказал волк».\n'
        '• «В один из ясных утра» — ОШИБКА. Только «Одним ясным утром».\n'
        '• «Войдя в лес, начался дождь» — деепричастие и подлежащее разные. '
        'Только «Войдя в лес, герой увидел...».\n'
        '• «Из горыа», «в горые» — несуществующие формы. «гора»: из горы, в горе, до горы.\n\n'
        'УПОТРЕБЛЕНИЕ ИМЁН — КРИТИЧНО:\n'
        f'• Имя героя — СТРОГО «{name}». Не сокращай, не меняй на уменьшительное без необходимости. '
        f'Если используешь уменьшительное — оно должно быть логичным и единственным вариантом.\n'
        f'• Родительный падеж имени «{name}»: проверь правило — после твёрдой согласной → «{name[:-1]}ы» '
        f'(если имя оканчивается на «а»; пример: Лера→Леры, Нина→Нины); '
        f'после шипящей/г,к,х → «{name[:-1]}и» (Маша→Маши, Даша→Даши). '
        f'НИКОГДА не добавляй «й» или лишние буквы: «{name}й» — ОШИБКА.\n'
        '• Имена не склоняются неправильно: «с Алисой» (не «с Алисей»), «для Коли» (не «для Колей»).\n\n'
        'СКЛОНЕНИЕ СУЩЕСТВИТЕЛЬНЫХ:\n'
        '• Топонимы не получают лишних букв: «из леса» (не «из лесыа»), «в горе» (не «в горые»).\n'
        '• Согласование прилагательных: "Звёздное Сияние" (ср.р.) — НЕ "Звёздный Сияние".\n'
        '• Числительные: "два дня", "три часа", "пять минут".\n'
        '• Предлоги движения: "пошёл В лес", "вышел ИЗ леса", "вернулся ДОМОЙ".\n'
        f'• Кличка животного «{animal}» должна быть {("женской" if gender == "female" else "мужской") if True else ""} '
        f'по роду слова «{animal}».\n'
        '• ТОЛЬКО РЕАЛЬНЫЕ РУССКИЕ СЛОВА — не изобретай несуществующих форм.\n'
        '• ФИНАЛЬНАЯ ПРОВЕРКА: прочитай текст вслух — каждое предложение должно звучать '
        'как живая грамотная русская речь. Проверь все глаголы прошедшего времени на согласование, '
        'все слова на «-ся/-сь» на правильность окончания.'
    )
